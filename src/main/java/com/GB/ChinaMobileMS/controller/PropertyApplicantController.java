...
public class PropertyApplicantController {
	...
	//在Spring中生成set get方法 自动获取propertyApplicantService对象
	@RequestMapping(value="/login", method=RequestMethod.POST)
	public ModelAndView login(User user, HttpSession session){
		
		String password = user.getPassword();
		//ModeAndView Spring中的一个方便跳转类  spring 进行解析
		user = propertyApplicantService.login(user.getUserName(), user.getPassword());
...
//		return new ModelAndView("main").addObject("username", user.getUserName());
	...
	public ModelAndView addPropertyApplicant(User user){
		
		String string = propertyApplicantService.addPropertyApplicant(user);
